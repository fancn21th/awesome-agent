...
这个仓库不再只是按“资源类型”罗列链接，而是按工程分层组织内容，让框架、模式、技能、运行时、评测和生产化问题各归其位，避免不同层级混在一起。

配套阅读：[Agent Level](AGENT_LEVEL_zh.md) | [RAG Level](RAG_LEVEL_zh.md) | [AI 使用技巧](AI_TRICKS_zh.md)
...
- [Agent 模式](#agent-模式)
- [框架与规范驱动系统](#框架与规范驱动系统)
- [运行时、工具链与生产运维](#运行时工具链与生产运维)
- [可观测性、评测与基准](#可观测性评测与基准)
- [Coding Agent、Skills 与 Agentic SDLC](#coding-agentskills-与-agentic-sdlc)
- [垂直 Agent 与应用](#垂直-agent-与应用)
- [学习路径](#学习路径)
- [研究、资讯与市场信号](#研究资讯与市场信号)
- [个人推荐](#个人推荐)

## 基础层

- [models.dev](https://models.dev/) - 用于探索 AI 模型、能力、上下文限制与价格的开源数据库。
...
## Coding Agent、Skills 与 Agentic SDLC

这一层按照需求、架构、编码、测试、评审、文档和发布的交付链路来理解 Agent 工程。
...
这是这个仓库下一步最值得扩展的层级，适合继续补充 CAD、制造、供应链、ERP、BI 和知识工程等行业 Agent。
